...
#
    ...
    trainX=trainX * (max - min + 1e-7) + min
    ...
    test_precdict=test_predict.astype(int)
    ...
    for k in range(train_size):
      if trainX[k][24][0]>=start_year and trainX[k][24][1]>=start_month and trainX[k][24][2]>=start_day:
        f=open("sample/"+str(map[int(repeat/3)][0])+"_"+str(map[int(repeat/3)][1])+".txt",'w')
        flag=1
        while trainX[k+m][24][0]<=end_year and trainX[k+m][24][1]<=end_month and trainX[k+m][24][2]<=end_day:
        #  print(trainX[k+m][24])
          date = (trainX[k+m][24][3]+2000)*10000+trainX[k+m][24][4]*100+trainX[k+m][24][5]
          f.write(str(date)+"|")
          for n in range(6,9):
            #print(str(trainX[k+m][24][n]))
            f.write(str(trainX[k+m][24][n])+"|")
          f.write(str(test_predict[k+m-1][0])+"|\n")
          m+=1
